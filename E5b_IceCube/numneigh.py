...
for n_neigh in range(1, 200):
    ...
    print("# = " ,n_neigh, ": AUC = ", roc_auc_score(expected, predicted_probs[:, 1]))
    auc[n_neigh] = roc_auc_score(expected, predicted_probs[:, 1])

fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(np.arange(1, 200), n_neigh, "-")
ax.set_ylabel("AUC")
ax.set_xlabel("Anzahl nächster Nachbarn")
ax.set_xlim([1,100])
fig.savefig("plots/number_neigh.pdf")
